...
async function seed() {
  ...
  try {
    ...
    if (riderCount < 10) {
      const mockRiders = Array.from({ length: 10 - riderCount }).map(
        (_, i) => ({
    ...
    for (let i = 0; i < 100; i++) {
      ...
      const parcel = {
        ...
        deliveryDate: new Date(creationDate.getTime() + 3 * 24 * 60 * 60 * 1000)
          .toISOString()
          .split("T")[0],
      ...
      if (status === "delivered") {
        parcel.delivered_at = new Date(
          creationDate.getTime() + (Math.random() * 48 + 24) * 60 * 60 * 1000,
        ).toISOString();
      }
