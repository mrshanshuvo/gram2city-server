...
export const updateParcelSchema = z.object({
  body: z.object({
    parcelName: z.string().min(2, 'Parcel name must be at least 2 characters').optional(),
    parcelType: z.string().optional(),
    weight: z.coerce.number().positive('Weight must be a positive number').optional(),
    receiverName: z.string().min(2, 'Receiver name must be at least 2 characters').optional(),
    receiverPhone: z
      .string()
      .regex(/^01[3-9]\d{8}$/, 'Invalid Bangladeshi phone number')
      .optional(),
